...
export const allPosts = query(async () => cms.posts.list({ limit: 50 }));

export const togglePublished = command(ToggleInput, async ({ id, published }) =>
	cms.posts.update(id, { published }),
);

export const createPost = command(cms.posts.schemas.create, async (input) =>
	cms.posts.create(input),
);

export const updatePost = command(cms.posts.schemas.update, async (input) => {
