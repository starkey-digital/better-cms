...
- **Dry-run before tagging a release that creates new packages:** `bun run release:check`. CI runs it on every PR, so packaging breaks surface before a tag rather than during one. It packs and resolves order without writing to the registry.
