...
- **`config.adapter` and `config.media` are factories: `(ctx: AdapterContext) => T | Promise<T>`.** Eager form is gone — the union no longer accepts a bare adapter. The factory receives `{ env }` from the handler and returns the constructed adapter. `cmsHandle(config, { env })` plumbs `$env/dynamic/private` (or any env object) through to the factory so the config module never reads `process.env` at module scope and stays safe for `<CMSAdmin {config}>` to import. `createCMS` calls each factory once at boot (`await config.adapter(ctx)`). No wrapper helper — the factory is the contract.
